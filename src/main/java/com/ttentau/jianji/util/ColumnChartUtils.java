...
//        axisX.setMaxLabelChars(81); //最多几个X轴坐标，意思就是你的缩放让X轴上数据的个数7<=x<=mAxisXValues.length
    ...
    public static ArrayList<Column> getColumn(List<List<SubcolumnValue>> p){
        ArrayList<Column> list = new ArrayList<>();
        for (int i = 0; i < p.size(); i++) {
            Column column = new Column(p.get(i));
            column.setHasLabels(true);//曲线的数据坐标是否加上备注
            list.add(column);
        }
        return list;
    }
    ...
    public static List<List<SubcolumnValue>> getP(ArrayList<ChartData> data,int color,boolean isDeleteLastData) {
        if (isDeleteLastData) data.remove(data.size()-1);
        List<List<SubcolumnValue>> list = new ArrayList<>();
        for (int i = 0; i < data.size(); i++) {
            List<SubcolumnValue> P = new ArrayList<>();
            P.add(new SubcolumnValue(data.get(i).money<0?0-data.get(i).money:data.get(i).money,color));
            list.add(P);
        }
        return list;
    }
